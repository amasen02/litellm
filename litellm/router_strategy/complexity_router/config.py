...
def split_classification_sections(
    classification_prompt: object, classification_examples: object
) -> tuple[object, object]:
    """Split a combined prompt written before the instructions and examples became separate fields.

    An earlier editor stored both sections in ``classification_prompt``, joined by the heading this
    module renders. Splitting on that exact marker, and only where the assembler could have emitted
    it (opening the value, or after a blank line), keeps those routers sending the same system
    prompt while the two fields now mean what they say. Anything else is left verbatim, so a heading
    an operator wrote mid-sentence is never reinterpreted as a section break.
    """
    if classification_examples is not None or not isinstance(classification_prompt, str):
        return classification_prompt, classification_examples
    instructions, marker, examples = classification_prompt.partition(f"{CALIBRATION_EXAMPLES_HEADING}\n")
    if not marker or not examples.strip() or (instructions and not instructions.endswith("\n\n")):
        return classification_prompt, classification_examples
    return instructions.strip() or None, examples
...
class ComplexityRouterConfig(BaseModel):
    ...
    @model_validator(mode="before")
    @classmethod
    def _split_legacy_classification_sections(cls, data: object) -> object:
        """Runs before the field validators so each half is capped against its own limit."""
        if not isinstance(data, dict):
            return data
        prompt: Final = data.get("classification_prompt")
        instructions, examples = split_classification_sections(prompt, data.get("classification_examples"))
        if instructions is prompt:
            return data
        return MappingProxyType({**data, "classification_prompt": instructions, "classification_examples": examples})
