...
class TestAutoRouterClassifierDefaultPrompt:
    ...
    @pytest.mark.asyncio
    async def test_a_prompt_written_before_the_split_previews_the_way_the_router_resolves_it(self):
        """The dashboard sends back whatever a stored router holds, so the preview has to apply the
        same load-time split the config does or it renders a doubled examples heading."""
        from litellm.router_strategy.complexity_router import ClassificationRubric, classification_system_prompt

        shipped = classification_system_prompt(5, classification_rubric=ClassificationRubric.AGENTIC)
        instructions, _, rest = shipped.partition("\n\nTiers:\n")
        examples = rest.split("Calibration examples:\n", 1)[1].split("\n\nThe message may quote", 1)[0]

        prompt = await self._preview(
            context_window_size=5,
            classification_prompt=f"{instructions}\n\nCalibration examples:\n{examples}",
            classification_rubric=ClassificationRubric.AGENTIC,
        )
        assert prompt == shipped
        assert prompt.count("Calibration examples:") == 1
